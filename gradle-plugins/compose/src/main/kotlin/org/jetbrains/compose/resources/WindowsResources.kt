...
internal fun Project.configureSyncWindowsComposeResources(
    ...
    kotlinExtension.targets.withType(KotlinNativeTarget::class.java)
        ...
        .all { target ->
            target.binaries.withType(Executable::class.java).all { binary ->
                configureSyncWindowsBinaryResources(target, binary)
            }
            target.binaries.withType(TestExecutable::class.java).all { binary ->
                configureSyncWindowsBinaryResources(target, binary)
            }
        }
}

private fun Project.configureSyncWindowsBinaryResources(
    ...
    val syncResources = tasks.registerOrConfigure<Sync>(
        "sync${binary.name.uppercaseFirstChar()}" +
    ...
    binary.linkTaskProvider.dependsOn(syncResources)
}
